...
exports.GetBlockHelperState = void 0;
const _1 = require("./");
...
class GetBlockHelperState {
    ...
    process(msg, context) {
        ...
        if (charCode == chars.EOS) {
            return {
                err: new StubbleError_1.StubbleError(errors.ERROR_UNTERMINATED_BLOCK, `Unterminated block helper "${this._helper}" at ${this._line}:${this._symbol}`),
            };
        }
        else if (charCode == chars.CLOSE_BRACKET) {
            return { state: new _1.CloseBracketState() };
        }
    ...
    notify(msg, context) {
        switch (msg.type) {
            case notifies.NOTIFY_SECOND_CLOSE_BRACKET_FOUND:
                return {
                    state: new _1.GetBlockEndState(this._helper),
                };
    ...
    result(context) {
        ...
        try {
            ...
            console.error(e);
            result.err = new StubbleError_1.StubbleError(errors.ERROR_CALLING_HELPER, `Helper "${this._helper}" error: ${e.toString()}`);
        }
        return result;
    }
}
exports.GetBlockHelperState = GetBlockHelperState;
//# sourceMappingURL=GetBlockHelperState.js.map